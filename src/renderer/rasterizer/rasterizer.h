	...
	inline void rasterizer<VB, RT>::draw(size_t num_vertexes, size_t vertex_offset) {
		...
		while(vertex_id < vertex_offset + num_vertexes) {
			...
			for (int y = min_aabb.y; y <= max_aabb.y; y++) {
				int2 point(x, y);
				float u = edge_function(vertex_b, vertex_c, point) / edge;
				float v = edge_function(vertex_c, vertex_a, point) / edge;
				float w = edge_function(vertex_a, vertex_b, point) / edge;
