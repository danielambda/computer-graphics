...
namespace cg {
	...
	template<typename T>
	inline T& resource<T>::item(size_t item) { return data.at(item); }

	template<typename T>
	inline T& resource<T>::item(size_t x, size_t y) { return data.at(x + stride * y); }

	template<typename T>
	inline size_t resource<T>::size_bytes() const { return item_size * data.size(); }

	template<typename T>
	inline size_t resource<T>::count() const { return data.size(); }
	...
	struct unsigned_color {
		...
		static unsigned_color from_color(const color& color) {
			return unsigned_color{
			  std::clamp(static_cast<uint8_t>(color.r * 255.f), kMin, kMax),
			  std::clamp(static_cast<uint8_t>(color.g * 255.f), kMin, kMax),
			  std::clamp(static_cast<uint8_t>(color.b * 255.f), kMin, kMax)
			};
		};

		static unsigned_color from_float3(const float3& color) {
			auto c = clamp(color * 255., 0., 255.);
			return unsigned_color{
		...
		float3 to_float3() const { return float3{r / 255.f, g / 255.f, b / 255.f}; };
